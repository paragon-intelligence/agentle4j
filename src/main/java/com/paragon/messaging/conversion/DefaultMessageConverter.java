 ...
 *
 * <p>Provides straightforward conversion between WhatsApp message types and the framework's
 * UserMessage/AssistantMessage types for AI context.
 *
 * <h2>Conversion Behavior</h2>
 *
 * <ul>
 *   <li><b>Text messages:</b> Body text extracted directly
 *   <li><b>Media messages:</b> Caption if present, otherwise type placeholder (e.g., "[Image]",
 *       "[Video: my_file.mp4]")
 *   <li><b>Interactive messages:</b> Button or list selection text
 *   <li><b>Location messages:</b> Coordinates description
 *   <li><b>System/Order messages:</b> Type placeholders
 * </ul>
 *
 * <h2>Usage Example</h2>
 *
...
public final class DefaultMessageConverter implements MessageConverter {
  ...
  private static final DefaultMessageConverter INSTANCE =
      new DefaultMessageConverter(DEFAULT_BATCH_SEPARATOR);
   ...
   *
   * <p>Uses newline as the batch separator when combining multiple messages.
   *
  ...
  @Override
  public @NonNull UserMessage toUserMessage(
  ...
  @Override
  public @NonNull List<UserMessage> toUserMessages(
      @NonNull List<? extends InboundMessage> inboundMessages) {
    Objects.requireNonNull(inboundMessages, "inboundMessages cannot be null");

    return inboundMessages.stream().map(this::toUserMessage).toList();
  }
  ...
  @Override
  public @NonNull List<AssistantMessage> toAssistantMessages(
    ...
    return outboundMessages.stream().map(this::toAssistantMessage).toList();
  }
  ...
  @Override
  public @NonNull OutboundMessage toOutboundMessage(
    ...
    return TextMessage.builder().body(aiResponse).replyTo(replyToMessageId).build();
  }
