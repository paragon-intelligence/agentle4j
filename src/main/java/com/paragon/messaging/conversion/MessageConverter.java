 ...
 *
 * <p>This interface provides bidirectional conversion between:
 *
 * <ul>
 *   <li>{@link InboundMessage} (from WhatsApp) &rarr; {@link UserMessage} (for AI)
 *   <li>{@link OutboundMessage} (to WhatsApp) &rarr; {@link AssistantMessage} (for history)
 *   <li>AI text response &rarr; {@link OutboundMessage} (for sending)
 * </ul>
 *
 * <h2>Usage Example</h2>
 *
...
public interface MessageConverter {
   ...
   *
   * <ul>
   *   <li>Text messages: body text
   *   <li>Media messages: caption or type placeholder ([Image], [Video], etc.)
   *   <li>Interactive messages: button/list selection text
   *   <li>Location messages: coordinates description
   * </ul>
   *
   ...
   *
   * <p>When users send multiple messages in quick succession (message batching), they should be
   ...
   *
   * <p>Use this when you need to preserve individual message boundaries in conversation history.
   *
   * @param inboundMessages the list of inbound messages
   * @return a list of UserMessages, one per inbound message
   */
  @NonNull List<UserMessage> toUserMessages(
      @NonNull List<? extends InboundMessage> inboundMessages);

  /**
   * Converts an outbound WhatsApp message to an AssistantMessage.
   *
   * <p>This is useful for building conversation history that includes assistant responses. The
   ...
   */
  @NonNull List<AssistantMessage> toAssistantMessages(
      @NonNull List<? extends OutboundMessage> outboundMessages);

  /**
   * Converts a plain AI text response to an OutboundMessage.
   *
   * <p>This is the simplest conversion, wrapping the AI's text response in a TextMessage ready for
   ...
   *
   * <p>The resulting message will quote/reply to the specified message ID when rendered in the
   * WhatsApp client.
   *
   * @param aiResponse the AI's text response
   ...
   */
  @NonNull OutboundMessage toOutboundMessage(
