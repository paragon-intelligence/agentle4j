 ...
 *
 * <p>Implementations provide persistence for conversation messages, enabling AI agents to maintain
 ...
 *
 * <ul>
 *   <li>Per-user message storage and retrieval
 *   <li>Configurable history limits (count and age)
 *   <li>Support for building AI context from history
...
public interface ConversationHistoryStore {
   ...
   *
   * <p>Messages are stored with a timestamp for age-based filtering. If the store has a maximum
   ...
   *
   * <p>Messages are added in order (first message is oldest).
   *
   ...
   *
   * <p>Returns messages in chronological order (oldest first), filtered by both count and age
   * limits. Messages older than {@code maxAge} are excluded, then the most recent {@code
   * maxMessages} are returned.
   *
   * @param userId the user's unique identifier
   ...
   *
   * <p>Use with caution - this removes all stored data.
   */
  void clearAll();
   ...
   *
   * <p>This method can be called periodically to clean up stale data. Some implementations may
