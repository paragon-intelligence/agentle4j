 ...
 *
 * <p>Stores conversation history in memory with configurable per-user limits
 * and LRU eviction. Suitable for development, testing, and single-instance
 * deployments where persistence across restarts is not required.</p>
 *
 * <h2>Features</h2>
 * <ul>
 *   <li>Thread-safe with fine-grained locking per user</li>
 *   <li>LRU eviction when per-user limit is reached</li>
 *   <li>Timestamped messages for age-based filtering</li>
 ...
 *
 * <h2>Thread Safety</h2>
 * <p>This implementation is thread-safe. Operations on different users can
 * proceed concurrently. Operations on the same user are serialized using
 * per-user read-write locks for optimal concurrent read performance.</p>
 *
 * @author Agentle Team
 * @since 2.1
...
public final class InMemoryConversationHistoryStore implements ConversationHistoryStore {
    ...
    public void addMessage(@NonNull String userId, @NonNull Message message) {
        ...
        UserHistory history = histories.computeIfAbsent(userId,
                k -> new UserHistory(maxMessagesPerUser));
        history.add(message);
    }
    ...
    private static final class UserHistory {
        ...
        List<ResponseInputItem> getRecent(int maxMessages, Duration maxAge) {
            ...
            try {
                ...
                List<ResponseInputItem> result = messages.stream()
    ...
    public static final class Builder {
         ...
         *
         * <p>When this limit is reached, older messages are evicted (LRU).</p>
         *
