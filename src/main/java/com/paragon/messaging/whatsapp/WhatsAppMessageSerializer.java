 ...
 *
 * <p>Esta implementação demonstra o padrão de como transformar os modelos genéricos em payloads
...
public class WhatsAppMessageSerializer {
  ...
  public String serialize(Recipient recipient, OutboundMessage message) {
    String json =
        switch (message) {
          ...
          case MediaMessageInterface media ->
              serializeMediaMessage(recipient, (MediaMessage) media);
          case TemplateMessageInterface template ->
              serializeTemplateMessage(recipient, (TemplateMessage) template);
          case InteractiveMessageInterface interactive ->
              serializeInteractiveMessage(recipient, (InteractiveMessage) interactive);
          case LocationMessageInterface location ->
              serializeLocationMessage(recipient, (LocationMessage) location);
          case ContactMessageInterface contact ->
              serializeContactMessage(recipient, (ContactMessage) contact);
          case ReactionMessageInterface reaction ->
              serializeReactionMessage(recipient, (ReactionMessage) reaction);
        };
    return json.strip();
  }
  ...
  private String serializeImage(Recipient recipient, MediaMessage.Image image) {
    ...
    return String.format(
        ...
        """,
        escapeJson(recipient.value()), sourceField, captionField);
  }
  ...
  private String serializeAudio(Recipient recipient, MediaMessage.Audio audio) {
    ...
    return String.format(
        ...
        """,
        escapeJson(recipient.value()), sourceField);
  }
  ...
  private String serializeDocument(Recipient recipient, MediaMessage.Document document) {
    ...
    return String.format(
        ...
        """,
        escapeJson(recipient.value()), sourceField, filenameField, captionField);
  }
  ...
  private String serializeTemplateComponent(TemplateMessage.TemplateComponent component) {
    return String.format(
        ...
        """,
        escapeJson(component.type()), serializeStringList(component.parameters()));
  }
    ...
    for (int i = 0; i < msg.buttons().size(); i++) {
      ...
      buttonsJson.append(
          String.format(
              ...
              """,
              escapeJson(btn.id()), escapeJson(btn.title())));
    }
    buttonsJson.append("\n        ]");
        ...
        sectionsJson.append(
            String.format(
                """
                {
                  ...
                  "title":"%s\"\
                """,
                escapeJson(row.id()), escapeJson(row.title())));
  ...
  private String serializeLocationMessage(Recipient recipient, LocationMessage location) {
    String nameField =
        location.name().map(n -> ",\n        \"name\":\"" + escapeJson(n) + "\"").orElse("");
    String addressField =
        location.address().map(a -> ",\n        \"address\":\"" + escapeJson(a) + "\"").orElse("");
  ...
  private String serializeContactMessage(Recipient recipient, ContactMessage contact) {
    ...
    for (int i = 0; i < contact.phones().size(); i++) {
      ...
      phonesJson
          .append("{\"phone\":\"")
          .append(escapeJson(p.phone()))
          .append("\",\"type\":\"")
          .append(escapeJson(p.type()))
          .append("\"}");
    }
    phonesJson.append("]");
    ...
    for (int i = 0; i < contact.emails().size(); i++) {
      ...
      emailsJson
          .append("{\"email\":\"")
          .append(escapeJson(e.email()))
          .append("\",\"type\":\"")
          .append(escapeJson(e.type()))
          .append("\"}");
    }
    emailsJson.append("]");
  ...
  private String serializeReactionMessage(Recipient recipient, ReactionMessage reaction) {
    String emojiField =
        reaction.emoji().map(e -> "\"emoji\":\"" + escapeJson(e) + "\"").orElse("\"emoji\":\"\"");
    ...
    return String.format(
        ...
        """,
        escapeJson(recipient.value()), escapeJson(reaction.messageId()), emojiField);
  }
