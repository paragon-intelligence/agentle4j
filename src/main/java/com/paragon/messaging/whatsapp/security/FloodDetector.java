package com.paragon.messaging.whatsapp.security;
 ...
 *
 * <p>Tracks message timestamps per user and blocks users who send
 * too many messages within a configured time window.</p>
 *
 * <h2>Features</h2>
 * <ul>
 *   <li>Per-user message rate tracking</li>
 *   <li>Configurable window and threshold</li>
...
public final class FloodDetector {
    ...
    public static FloodDetector create(@NonNull SecurityConfig config) {
        ...
        return new FloodDetector(
                config.floodPreventionWindow(),
     ...
     *
     * <p>This method does NOT record a new message. Use {@link #recordMessage(String)}
     * after processing to track the message.</p>
     *
     ...
     *
     * <p>Call this after processing a message to track it for rate limiting.</p>
     *
     * @param userId the user identifier
     */
     ...
     *
     * <p>Returns true if the user WAS flooding before this message.
     * The message is recorded regardless.</p>
     *
     * @param userId the user identifier
    ...
    public boolean checkAndRecord(@NonNull String userId) {
        ...
        UserMessageHistory history = userHistories.computeIfAbsent(
                userId, k -> new UserMessageHistory());
     ...
     *
     * <p>Call this periodically (e.g., every minute) to prevent memory growth.</p>
     *
     * @return number of users cleaned up (had all entries removed)
     */
    public int cleanup() {
        int cleaned = 0;
        Iterator<Map.Entry<String, UserMessageHistory>> iterator =
                userHistories.entrySet().iterator();
