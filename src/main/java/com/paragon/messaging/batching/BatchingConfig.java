 ...
 *
 * <p>Aggregates all configurations for batching, rate limiting, backpressure, error handling, TTS,
...
public record BatchingConfig(
    @NonNull @NotNull(message = "Adaptive timeout cannot be null") Duration adaptiveTimeout,
    @NonNull @NotNull(message = "Silence threshold cannot be null") Duration silenceThreshold,
    @Positive(message = "Max buffer size must be positive")
        ...
        int maxBufferSize,
    @NonNull @NotNull(message = "Rate limit config cannot be null") @Valid
        RateLimitConfig rateLimitConfig,
    @NonNull @NotNull(message = "Backpressure strategy cannot be null")
        BackpressureStrategy backpressureStrategy,
    @NonNull @NotNull(message = "Error handling strategy cannot be null") @Valid
        ErrorHandlingStrategy errorHandlingStrategy,
    @Nullable @Valid MessageStore messageStore,
    @NonNull @NotNull(message = "TTS config cannot be null") @Valid TTSConfig ttsConfig,
    @Nullable @Valid SecurityConfig securityConfig) {
  ...
  public BatchingConfig {
    ...
    if (adaptiveTimeout != null
        ...
        && silenceThreshold.compareTo(adaptiveTimeout) > 0) {
   ...
   *
   * <ul>
   *   <li>Adaptive timeout: 5 seconds
   *   <li>Silence: 1 second
   *   <li>Buffer: 50 messages
   *   <li>Rate limit: Lenient
   *   <li>Backpressure: DROP_OLDEST
   *   <li>Error handling: 3 retries with exponential backoff
   *   <li>No TTS, no persistence, no security
   * </ul>
   *
  ...
  public static final class Builder {
     ...
     *
     * <p>Even if messages continue arriving, after this time the buffer is processed.
     *
     ...
     *
     * <p>If the user stops sending for this duration, the buffer is processed immediately (doesn't
     ...
     *
     * <p>When reached, backpressureStrategy is applied.
     *
