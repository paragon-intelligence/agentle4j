...
public final class Agent implements Serializable, Interactable {
  ...
  private Agent(Builder builder) {
    ...
    this.instructions =
        augmented ? Prompt.of(instructionAugmentation.toString()) : baseInstructions;
   ...
   *
   * @return the reasoning config or null
   */
  ...
  public Interactable.@NonNull Streaming asStreaming() {
    return new Interactable.Streaming() {
      ...
      @Override
      public @NonNull AgentStream interact(@NonNull String input, @Nullable TraceMetadata trace) {
        AgenticContext context = AgenticContext.create();
  ...
  @NonNull AgentResult finalizeHandoffResult(
  ...
  private Object resolvePropagatedParsedOutput(
    ...
    return StructuredOutputSupport.parse(
        propagatedOutputDefinition, innerResult.output(), objectMapper);
  }
  ...
  /**
   * Extracts text from the conversation input for tool search. Uses the most recent user message as
   * the search query.
   */
  ...
  public static final class Builder {
     ...
     * each API call based on the user's input, using the registry's search strategy. This is useful
     * when the agent has many tools and sending all of them would waste context tokens.
     *
     * <p>Tools configured via {@link #addTool} are always included (eager). The registry's deferred
     * tools are only included when the search strategy deems them relevant.
     *
     ...
     */
    public @NonNull Builder toolRegistry(@NonNull ToolRegistry toolRegistry) {
     ...
     *
     * <p>Each skill is added to the agent's catalog and exposed via the {@code read_skill} tool for
     * on-demand loading.
     *
     * <p>Example:
     *
     ...
     *
     * <ul>
     *   <li>{@code beforeRun} — before the agentic loop starts
     *   <li>{@code afterRun} — after the agentic loop completes
     *   <li>{@code beforeToolCall} — before each tool execution
     *   <li>{@code afterToolCall} — after each tool execution
     * </ul>
     *
     ...
     *
     * @param reasoning the reasoning config (effort + summary kind)
     * @return this builder
     */
  ...
  public static final class StructuredBuilder<T> {
    ...
    public @NonNull Structured<T> build() {
      ...
      return new Structured<T>(agent, structuredOutputDefinition);
    }
  }
  ...
  public static final class Structured<T> implements Interactable.Structured<T> {
    ...
    private @NonNull StructuredAgentResult<T> parseResult(AgentResult result) {
      ...
      try {
        T parsed =
            StructuredOutputSupport.parse(
                structuredOutputDefinition, result.output(), objectMapper);
        return StructuredAgentResult.success(
            parsed, AgentResult.Builder.from(result).parsed(parsed).build());
      } catch (JacksonException e) {
