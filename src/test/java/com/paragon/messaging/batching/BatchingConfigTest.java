...
class BatchingConfigTest {
  ...
  class BuilderTests {
    ...
    void builder_setsRateLimitConfig() {
      RateLimitConfig rateLimitConfig = RateLimitConfig.builder().tokensPerMinute(20).build();

      BatchingConfig config = BatchingConfig.builder().rateLimitConfig(rateLimitConfig).build();
    ...
    void builder_setsErrorHandlingStrategy() {
      ErrorHandlingStrategy errorStrategy = ErrorHandlingStrategy.builder().maxRetries(5).build();

      BatchingConfig config = BatchingConfig.builder().errorHandlingStrategy(errorStrategy).build();
  ...
  class ValidationTests {
    ...
    void nestedRateLimitConfig_isValidated() {
      ...
      BatchingConfig config =
          BatchingConfig.builder().rateLimitConfig(invalidRateLimitConfig).build();
    ...
    void nestedErrorHandlingStrategy_isValidated() {
      ...
      BatchingConfig config =
          BatchingConfig.builder().errorHandlingStrategy(invalidStrategy).build();
  ...
  class AccessorTests {
    ...
    void adaptiveTimeout_returnsValue() {
      ...
      BatchingConfig config = BatchingConfig.builder().adaptiveTimeout(timeout).build();
    ...
    void silenceThreshold_returnsValue() {
      ...
      BatchingConfig config = BatchingConfig.builder().silenceThreshold(threshold).build();
    ...
    void maxBufferSize_returnsValue() {
      int size = 25;
      BatchingConfig config = BatchingConfig.builder().maxBufferSize(size).build();
    ...
    void backpressureStrategy_returnsValue() {
      ...
      BatchingConfig config = BatchingConfig.builder().backpressureStrategy(strategy).build();
