...
class ConfigurationValidationTest {
  ...
  void testBatchingConfigValidation() {
    ...
    assertTrue(
        violations.stream().anyMatch(v -> v.getPropertyPath().toString().equals("maxBufferSize")));
  }

  @Test
  void testRateLimitConfigValidation() {
    // Test with invalid tokensPerMinute (> 10000)
    RateLimitConfig invalidConfig =
  ...
  void testErrorHandlingStrategyValidation() {
    ...
    Set<ConstraintViolation<ErrorHandlingStrategy>> violations =
        validator.validate(invalidStrategy);
    assertFalse(violations.isEmpty());
    assertTrue(
  ...
  void testTTSConfigValidation() {
    ...
    assertFalse(violations.isEmpty());
    assertTrue(
        violations.stream().anyMatch(v -> v.getPropertyPath().toString().equals("speechChance")));
  }
  ...
  void testTTSConfigInvalidLanguageCode() {
    ...
    assertFalse(violations.isEmpty());
    assertTrue(
        violations.stream().anyMatch(v -> v.getPropertyPath().toString().equals("languageCode")));
  }
  ...
  void testNestedValidation() {
    ...
    BatchingConfig configWithInvalidNested =
        BatchingConfig.builder()
            .rateLimitConfig(
                RateLimitConfig.builder()
    ...
    Set<ConstraintViolation<BatchingConfig>> violations =
        validator.validate(configWithInvalidNested);
    assertFalse(violations.isEmpty());
