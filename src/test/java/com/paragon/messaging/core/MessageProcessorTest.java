...
class MessageProcessorTest {
    ...
    void setUp() {
        ...
        testMessages = Arrays.asList(
                createTextMessage("wamid.1", "Hello"),
                createTextMessage("wamid.2", "How are you?")
        );
    }
    ...
    class FactoryMethodsTests {
        ...
        void loggingCreatesProcessor() throws Exception {
            ...
            MessageProcessor processor = MessageProcessor.logging(log -> {
    ...
    class ProcessMethodTests {
        ...
        void processWithContextReceivesParameters() throws Exception {
            ...
            MessageProcessor.ProcessingContext[] receivedContext = new MessageProcessor.ProcessingContext[1];
            ...
            MessageProcessor.ProcessingContext context = MessageProcessor.ProcessingContext.create(
        ...
        void processSimplifiedDelegates() throws Exception {
            ...
            assertEquals(MessageProcessor.ProcessingReason.UNKNOWN, receivedContext[0].processingReason());
        }
    ...
    class ProcessingReasonTests {
        ...
        void hasTimeoutReason() {
            assertEquals(MessageProcessor.ProcessingReason.TIMEOUT,
        ...
        void hasSilenceReason() {
            assertEquals(MessageProcessor.ProcessingReason.SILENCE,
        ...
        void hasBufferFullReason() {
            assertEquals(MessageProcessor.ProcessingReason.BUFFER_FULL,
        ...
        void hasUnknownReason() {
            assertEquals(MessageProcessor.ProcessingReason.UNKNOWN,
    ...
    class RealWorldScenariosTests {
        ...
        void handlesBatchTimeout() throws Exception {
            ...
            MessageProcessor processor = (userId, messages, context) ->
                    receivedReason[0] = context.processingReason();
        ...
        void handlesRetryAfterFailure() throws Exception {
            ...
            MessageProcessor.ProcessingContext firstContext = MessageProcessor.ProcessingContext.create(
                    "batch-123", "msg-1", "msg-2", MessageProcessor.ProcessingReason.TIMEOUT);

            // First attempt fails
            assertThrows(RuntimeException.class, () ->
                    processor.process(testUserId, testMessages, firstContext));
            assertEquals(1, attempts.get());

            // Retry succeeds
            MessageProcessor.ProcessingContext retryContext = firstContext.retry();
            assertDoesNotThrow(() ->
                    processor.process(testUserId, testMessages, retryContext));
    ...
    class FunctionalInterfaceTests {
        ...
        void canBeComposed() throws Exception {
            ...
            MessageProcessor baseProcessor = (userId, messages, context) ->
                    counter.incrementAndGet();

            MessageProcessor loggingWrapper = (userId, messages, context) -> {
