...
class MessageResponseTest {
    ...
    class ValidationTests {
        ...
        void validatesMessageIdNotBlank() {
            ...
            assertTrue(violations.stream()
                    .anyMatch(v -> v.getMessage().contains("Message ID cannot be blank")));
        }
    ...
    class MessageStatusTests {
        ...
        void hasAcceptedStatus() {
            assertEquals(MessageResponse.MessageStatus.ACCEPTED,
        ...
        void hasSentStatus() {
            assertEquals(MessageResponse.MessageStatus.SENT,
                    MessageResponse.MessageStatus.valueOf("SENT"));
        }

        @Test
        @DisplayName("has DELIVERED status")
        void hasDeliveredStatus() {
            assertEquals(MessageResponse.MessageStatus.DELIVERED,
        ...
        void hasReadStatus() {
            assertEquals(MessageResponse.MessageStatus.READ,
                    MessageResponse.MessageStatus.valueOf("READ"));
        }

        @Test
        @DisplayName("has FAILED status")
        void hasFailedStatus() {
            assertEquals(MessageResponse.MessageStatus.FAILED,
                    MessageResponse.MessageStatus.valueOf("FAILED"));
        }

        @Test
        @DisplayName("has UNKNOWN status")
        void hasUnknownStatus() {
            assertEquals(MessageResponse.MessageStatus.UNKNOWN,
                    MessageResponse.MessageStatus.valueOf("UNKNOWN"));
        }
    }
    ...
    class RealWorldScenariosTests {
        ...
        void handlesReadReceipt() {
            MessageResponse response = new MessageResponse(
                    "wamid.ABC123",
                    MessageResponse.MessageStatus.READ,
                    Instant.now()
            );
