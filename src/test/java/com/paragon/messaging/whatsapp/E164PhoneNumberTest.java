...
class E164PhoneNumberTest {
  ...
  class ValidationTests {
    ...
    void rejectsStartingWithZero() {
      ...
      assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("cannot start with 0")));
    }
    ...
    void rejectsTooLong() {
      ...
      assertFalse(violations.isEmpty());
      assertTrue(
          violations.stream().anyMatch(v -> v.getMessage().contains("between 2 and 15 digits")));
    }
    ...
    void rejectsWithLetters() {
      ...
      Set<ConstraintViolation<PhoneHolder>> violations = validator.validate(holder);
      assertFalse(violations.isEmpty());
      assertTrue(violations.stream().anyMatch(v -> v.getMessage().contains("only digits")));
    }
