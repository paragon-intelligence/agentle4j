 ...
 * 
 * <p>Verifies that {@code Agent.interact()} auto-pauses when encountering
 * a tool with {@code requiresConfirmation=true} and can be resumed with
 * {@code Agent.resume()}.
 */
...
class AgentNonStreamingHitlTest {
  ...
  static class DangerousTool extends FunctionTool<SimpleParams> {
    ...
    public FunctionToolCallOutput call(SimpleParams params) {
      executed = true;
      return FunctionToolCallOutput.success("dangerous_call", "Dangerous result: " + params.value());
    }
  }
  ...
  class AutoPauseBehaviorTests {
    ...
    void interactPausesForToolRequiringConfirmation() {
      ...
      mockWebServer.enqueue(new MockResponse()
          .setResponseCode(200)
          .setBody(buildToolCallResponse("dangerous_tool")));
    ...
    void interactDoesNotPauseForSafeTools() {
      ...
      mockWebServer.enqueue(new MockResponse()
          .setResponseCode(200)
          .setBody(buildToolCallResponse("safe_tool")));
      
      // After tool execution, LLM gives final answer
      mockWebServer.enqueue(new MockResponse()
          .setResponseCode(200)
          .setBody(buildTextResponse("Done!")));
  ...
  class EdgeCasesTests {
    ...
    void resumeThrowsIfNotPending() {
      ...
      AgentRunState completedState = AgentRunState.completed(
          "TestAgent", AgentContext.create(), null, List.of(), 1);
